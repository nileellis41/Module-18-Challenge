...
new_block = Block(data="Record", creator_id=42)
...
# Assume PyChain is defined and initialized as in previous discussions

# Create input fields for Record attributes
st.markdown("# PyChain: A Simple Blockchain Ledger")
st.markdown("## Create a new transaction")
sender = st.text_input("Sender")
receiver = st.text_input("Receiver")
amount = st.text_input("Amount")

# Button to add a new block
if st.button("Add Transaction to Ledger"):
    if sender and receiver and amount:
        try:
            amount = float(amount)
            new_record = Record(sender, receiver, amount)
            # Assuming the blockchain's last block's hash and creator_id is handled internally
            if pychain.chain:
                prev_hash = pychain.chain[-1].hash_block()
            else:
                prev_hash = "0"
            new_block = Block(record=new_record, creator_id=42, prev_hash=prev_hash)
...
# Display the current blockchain ledger
st.markdown("## Current Ledger")
pychain_df = pd.DataFrame([{
...
class Record:
    ...
    # Define an attribute named `send` with a type of `str`.
    sender: str
    # Define an attribute named `reciever` with a type of `str`.
    reciever: int
    # Define an attribute named `amount` with a type of `float`.
    amount: float
...
sender = st.text_input("Sender")
receiver = st.text_input("Receiver")
amount = st.text_input("Amount", value="0.00")  # Defaulting amount to 0.00 for better UX
